...
var secureClient = new SecureClient(client);

await secureClient.ConnectAsync(
  new IPEndPoint(IPAddress.Parse("192.168.31.191"), 1211),
...
_ = secureClient.HandleResponseFromServer();

while (true)
{
  await secureClient.SendMessageAsync(Console.ReadLine()!);
}
